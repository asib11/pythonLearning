...
#finding = soup.find('p') if i use only find, it returns first result of element
#finding = soup.find_all('p') it gives list
finding = soup.find_all(id= 'mobile')
finding2 = soup.select('#mobile') #substitude of id when uses select method
print(finding)
print(finding2)
class_find = soup.find_all(class_= 'copy') #when use bs4 always class_
print(class_find)

'''
for
 id use #id_name
class of bar .class_name
children div > p
decendent div p
'''